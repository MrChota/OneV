package com.connectstudios.connect.vpn
...
import com.connectstudios.connect.storage.ProxyLog
import com.connectstudios.connect.storage.ProxyLogDatabase
...
open class SimpleVpnService : VpnService() {
    ...
    private external fun tunnelConfig(): String

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        configString = JSONObject((tunnelConfig().replace("\\r\\n\\t", "").replace("\\r\\n", "").replace("\\",""))).toString(2).trimIndent()
        bgThread = thread(start = true) {
            ...
            if (config != null) {
                if (config.dns?.servers == null || config.dns.servers.isEmpty()) {
            ...
            val builder = Builder().setSession("Connect Vpn Session")
                .setMtu(1500)
            ...
            if (isEnablePerAppVpn) {
                ...
                when (perAppMode?.let { Integer.parseInt(it) }) {
                    0 -> {
            ...
            val sniffings = ArrayList<String>()
            if (sniffingList != null) {
